...
export default function KanbanCardCompleted({
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white w-full p-4 font-main shadow-sm hover:shadow-md transition-shadow duration-200 ease-in-out">
      ...
      {(school || faculty) && (
        <div className="text-xs font-light text-gray-600 mb-2 space-y-0.5">
          {school && (
            <p className="text" title={school}>
              {school}
            </p>
          )}
          {faculty && (
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent
          side="right"
          className="w-full sm:w-[560px] p-0 border-l bg-white"
        >
          <div className="bg-gradient-to-r from-blue-50 to-white border-b">
            <div className="px-6 py-4">
              <SheetHeader className="space-y-1">
                <SheetTitle className="text-xl font-playfair font-semibold text-gray-900 truncate">
                  {title || "Application Details"}
                </SheetTitle>
                <SheetDescription className="text-gray-600 text-sm">
                  Review the data that we will use to personalize your outreach.
                </SheetDescription>
              </SheetHeader>
            </div>
          </div>
          ...
          <div className="px-6 py-5 max-h-[75vh] overflow-y-auto space-y-6">
            ...
            <div className="rounded-lg border border-slate-200 p-4">
              <div className="flex items-center text-sm font-semibold text-gray-800 mb-3">
                <Briefcase className="h-4 w-4 mr-2 text-gray-600" /> Academic
                Affiliations
              </div>
              <div className="space-y-1 text-sm text-gray-800">
                ...
                {labs && (
                  <p className="truncate" title={labs}>
                    {labs}
                  </p>
                )}
                {lab_url && (
                  <Link
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center text-blue-600 text-sm mt-1 underline"
                  >
                    <ExternalLink className="h-3.5 w-3.5 mr-1" /> Visit lab
                    website
                  </Link>
                )}
              </div>
            ...
            {interests.length > 0 && (
              <div className="rounded-lg border border-slate-200 p-4">
                <div className="flex items-center text-sm font-semibold text-gray-800 mb-3">
                  <Tag className="h-4 w-4 mr-2 text-gray-600" /> Research
                  Interests
                </div>
              ...
              <div className="rounded-lg border border-slate-200 p-4">
                ...
                <div className="flex flex-wrap gap-2 items-center">
                  ...
                  {email && (
                    <button
                      onClick={copyEmail}
                      className="inline-flex items-center text-xs font-medium text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 px-2 py-1 rounded-md"
                    >
