  ...
  return (
    <>
      <Card className="m-10">
                ...
                {interests
                  ...
                  .map((interest, index) => {
                    ...
                    return (
                      <span
                        key={index}
                        className={`px-2 rounded-md ${colour.bg} ${colour.text}`}
                      >
                        {interest.trim()}
                      </span>
                    );
          ...
          <div className="flex">
            ...
            <div className="flex items-center text-sm space-x-1 w-full">
              <div className="text-sm flex items-center space-x-2 border-1 rounded-md w-full px-3">
                <span className="pr-1 font-semibold bg-purple-200 text-purple-800 px-1 rounded-2xl">
                  {name}
                </span>
                <Input
        ...
        <div>
          <Input
            className="p-2 border- rounded-none font-sans text-sm text-black"
            defaultValue="University of Toronto Resarch Inquiry"
          />
          <EmailTextEditor research_interests = {interests}/>
          {uploadedFile && (
            <div className="w-fit p-3 flex justify-center items-center space-x-4 ml-6 text-sm text-gray-500 border-1 bg-gray-50 font-sans rounded-md">
              ...
              <button onClick={deleteFile} className="cursor-pointer text-lg">
                X
              </button>
            </div>
          )}
